module ProjectsHelper
  ...
  def sess_sched_toggle (req, sess, trid, timeslot)
    ...
    if (assg)
      ...
      r += "<a href=\"#\" onclick=\"return update_tr('#{trid}', '#{link}', 10)\">#{req.project.display_name}</a>"
    else
      ...
      if (conflict.size > 1 and !sched.empty?)
        ...
        for item in sched
          ...
          r += "<a href=\"#\" onclick=\"return update_tr('#{trid}', '#{link}', 10)\">#{item.sess.project.display_name}</a> "
        end
      elsif (!sched.empty?)
        # Single conflict.
        ...
        r = "<span class=\"conflict\">"
        r += "<a href=\"#\" onclick=\"return update_tr('#{trid}', '#{link}', 10)\">#{conflict.first.sess.project.display_name}</a>"
        ...
        r += "<a href=\"#\" onclick=\"return update_tr('#{trid}', '#{link}', 10)\">+</a>"
      end
    end
    r += "</span>"
    return r
